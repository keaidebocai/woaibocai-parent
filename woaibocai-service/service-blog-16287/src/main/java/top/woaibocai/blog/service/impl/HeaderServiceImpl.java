...
public class HeaderServiceImpl implements HeaderService {
    ...
    public Result getCategoryAndMenu() {
        // getAllCategory
        // 查询 redis 有没有
        Map<String, String> entries = hashOperationSSS.entries("blog:header:getAllCategory");
        // 没有 就初始化
        if (entries.isEmpty()) {
            ...
            hashOperationSSS.putAll("blog:header:getAllCategory",cateCategoryMap);
        ...
        // getAllMenu
        // 查询redis
        List<Menu> menuList = redisTemplateObject.opsForList().range("blog:header:getAllMenu", 0,-1);
        if (menuList.isEmpty()) {
            List<Menu> allMenu = menuMapper.getAllMenu();
            redisTemplateObject.opsForList().leftPushAll("blog:header:getAllMenu",allMenu);
            menuList = allMenu;
        }
